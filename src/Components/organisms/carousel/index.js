       ...
       {carouselContent.map((item, index) => {
          return (
             (index === activeContent) && <div className="carousel" key={index}>
                <Link text={<Image location="/previous.png" alt="Discovery" width="20"/>} onClick={prevContent} style="nav-icon text-nav"/>
                <CarouselText textContent={item} key={index}/>
                <CarouselImage imageContent={item.image} />
               <Link text={<Image location="/next.png" alt="Discovery" width="20"/>} url="/" onClick={nextContent} style="nav-icon text-nav"/>
                  </div>
                    );
                })}
